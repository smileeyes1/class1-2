...
const fail=[];const warnings=[];const report={schema:'kefayat-regression-v2',checks:[]};
const c=JSON.parse(await readFile('zaytoona/kefayat/catalog.json','utf8'));const rows=c.records||[];
const seen=new Map(),dup=[];for(const r of rows){const k=r.semanticKey||`${r.source}|${r.grade}|${r.raw}`;if(seen.has(k))dup.push({key:k,first:seen.get(k),duplicate:r.id});else seen.set(k,r.id)}
...
const expected=['arabic','math','islamic','nurturing'];const matrix={};for(const g of [1,2,3,4])for(const s of expected){const n=rows.filter(r=>Number(r.grade)===g&&r.source===s).length;matrix[`G${g}|${s}`]=n;if(!n)warnings.push(`SOURCE_GAP:${g}:${s}`)}
report.checks.push({name:'grade-subject-coverage',pass:true,matrix,warnings:warnings.filter(x=>x.startsWith('SOURCE_GAP:'))});
...
const subjectValues=[...new Set(rows.map(r=>r.source))];report.checks.push({name:'subjects',pass:expected.every(x=>subjectValues.includes(x)),found:subjectValues});if(!expected.every(x=>subjectValues.includes(x)))fail.push('SUBJECTS');
