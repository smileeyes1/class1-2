...
SRC=ROOT/'.audit-source'/'kk'
OUT=ROOT/'zaytoona'/'kefayat'
SUBJECTS={
 'arabic':'اللغة العربية','math':'الرياضيات','islamic':'التربية الإسلامية','nurturing':'التنشئة/العلوم الحياتية'
}
EXPECTED={(g,s) for g in range(1,5) for s in SUBJECTS}
...
def grade(name):
 ...
 x=next(v for v in m.groups() if v)
 return {'١':1,'٢':2,'٣':3,'٤':4}.get(x, int(x) if x.isdigit() else None)
def subject(name):
 n=name.lower()
...
def header_score(cells):
 keys=['كفاية','الكفاية','معيار','المعيار','مؤشر','المؤشر','قيمة','القيم','مجال','المجال']
 return sum(1 for c in cells if any(k in c for k in keys))
def map_fields(headers,cells):
 r={}
 for h,c in zip(headers,cells):
  h=norm(h); c=norm(c)
  if not h: continue
  if any(k in h for k in ['مجال','المجال']): r.setdefault('domain',c)
  elif 'كفاية' in h: r.setdefault('competency',c)
  elif 'معيار' in h: r.setdefault('standard',c)
  elif 'مؤشر' in h: r.setdefault('indicator',c)
  elif 'قيمة' in h: r.setdefault('value',c)
  else: r.setdefault('extra',[]); r['extra'].append({'header':h,'text':c})
 return r

def parse_doc(path):
 g=grade(path.name); s=subject(path.name)
 if not g or not s: return [], {'file':path.name,'status':'UNMAPPED','grade':g,'subject':s}
 doc=Document(path); records=[]; tables=0
 for ti,t in enumerate(doc.tables):
  tables+=1; rows=[[norm(c.text) for c in row.cells] for row in t.rows]
  rows=[r for r in rows if any(r)]
  if not rows: continue
  hi=max(range(min(len(rows),4)), key=lambda i: header_score(rows[i]))
  headers=rows[hi]
  if header_score(headers)<2: headers=[f'column_{i+1}' for i in range(len(rows[hi]))]; hi=-1
  for ri,cells in enumerate(rows[hi+1:], start=hi+1):
   if len(cells)!=len(headers) or not any(cells): continue
   f=map_fields(headers,cells)
   meaningful=any(f.get(k) for k in ['competency','standard','indicator','value','domain'])
 ...
 return records, {'file':path.name,'status':'OK','grade':g,'subject':s,'tables':tables,'records':len(records)}

def main():
 if not SRC.exists():
  SRC.parent.mkdir(parents=True,exist_ok=True); subprocess.run(['git','clone','--depth','1','https://github.com/smileeyes1/kk.git',str(SRC)],check=True)
 files=list(SRC.glob('*.docx'))
 allr=[]; manifests=[]
 for p in files:
 ...
 for r in allr:
  ...
  by_comp.setdefault(k,set()).add((r['standard'],r['indicator'],r['value']))
 ...
 catalog={'schemaVersion':4,'audit':'Ω KEFAYAT MASTER AUDIT & COMPLETION','generatedFrom':{'repository':'smileeyes1/kk','branch':'main','localCheckout':str(SRC)},'officialityPolicy':'The repository is treated as a user-provided reference. No record is labelled officially verified unless independent evidence is available.','recordCount':len(records),'inputRecordCount':len(allr),'deduplicated':len(allr)-len(records),'coverage':{'grades':[1,2,3,4],'subjects':list(SUBJECTS),'matrix':{f'{g}|{s}':coverage[(g,s)] for g,s in sorted(EXPECTED)}},'records':records}
 ...
 (OUT/'catalog.min.json').write_text(json.dumps(catalog,ensure_ascii=False,separators=(',',':'))+'\n',encoding='utf-8')
 status='PASS' if not gaps else 'NO-GO'
 print(json.dumps({'status':status,'files':len(files),'records':len(records),'duplicates':len(duplicates),'conflicts':len(conflicts),'gaps':gaps,'coverage':catalog['coverage']['matrix']},ensure_ascii=False,indent=2))
 if gaps: sys.exit(2)
