...
test('Kefayat catalog preserves configured source domains', () => {
  ...
  assert.deepEqual(catalog.subjects, allSubjects);
  assert.equal(catalog.sourceMeta.length, allSubjects.length);
  assert.ok(Array.isArray(catalog.sourceDocuments));
});

test('Kefayat competency catalog is non-empty and covers grades 1-4', () => {
  ...
  for (const subject of competencySubjects) assert.ok(catalog.records.some(r => r.subject === subject), `missing competency source ${subject}`);
});
...
test('Kefayat competency records have stable identity and core fields', () => {
  ...
  for (const r of catalog.records) {
    ...
    ids.add(r.id);
    assert.ok(competencySubjects.includes(r.subject), `unknown competency subject: ${r.subject}`);
...
test('source metadata and index documents are internally consistent', () => {
  for (const meta of catalog.sourceMeta) {
    ...
    if (meta.kind === 'competency') assert.ok(meta.records > 0, `no records extracted: ${meta.file}`);
  }
  for (const doc of catalog.sourceDocuments) {
    assert.equal(doc.subject, 'islamic_education');
    assert.ok(doc.bytes > 0 && doc.text.length > 0, `empty index document: ${doc.file}`);
    assert.ok(doc.sha256?.length === 64, `missing index hash: ${doc.file}`);
  }
});
