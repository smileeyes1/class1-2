...
function isHeader(cols){ return /المجال.*كفايات|domain.*compet/i.test(cols.slice(0,6).join(' ')); }
function parse(text, subject){
  ...
  for(const raw of text.split(/\r?\n/)){
    ...
    if(cols.length<9 || isSeparator(cols) || isHeader(cols)) continue;
    const meaningful=cols.slice(0,5).filter(Boolean).length;
    if(meaningful<3 || !grade || !table) continue;
    rows.push({id:`${subject}:${grade}:${table}:${rows.length+1}`,subject,grade,table,sourceFile:SOURCES.find(x=>x[0]===subject)?.[1]||'',domain:cols[0]||'',practice:cols[1]||'',subcompetency:cols[2]||'',standard:cols[3]||'',learningOutcome:cols[4]||'',mastery:cols[5]||'',developing:cols[6]||'',attempting:cols[7]||'',attempting2:cols[8]||'',values:cols[9]||'',sourceColumns:cols});
  }
  return rows;
}
...
const catalog={schemaVersion:'1.1.0',generatedAt:new Date().toISOString(),source:{repository:'smileeyes1/kefayat',branch:'main',baseUrl:BASE,authority:'external-reference; preserve source text'},subjects:SOURCES.map(x=>x[0]),grades,sourceMeta,recordCount:all.length,records:all};
