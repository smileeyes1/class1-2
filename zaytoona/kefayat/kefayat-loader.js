...
(function(){
  ...
  function parse(text,subject,label,file){let grade=null,table=0,rows=[];for(const raw of text.split(/\r?\n/)){const line=raw.trim(),g=gradeFrom(line);if(g!==null){grade=g;continue}if(/^\[TABLE\s*\d+\]/i.test(line)){table++;continue}if(!line.includes('|')||line.startsWith('#'))continue;const c=line.split('|').map(x=>x.trim());if(c.length<9||isSeparator(c)||isHeader(c))continue;if(c.slice(0,5).filter(Boolean).length<3||!grade||!table)continue;rows.push({id:`${subject}:${grade}:${table}:${rows.length+1}`,subject,subjectLabel:label,grade,table,sourceFile:file,domain:c[0]||'',practice:c[1]||'',subcompetency:c[2]||'',standard:c[3]||'',learningOutcome:c[4]||'',mastery:c[5]||'',developing:c[6]||'',attempting:c[7]||'',attempting2:c[8]||'',values:c[9]||'',sourceColumns:c})}return rows}
